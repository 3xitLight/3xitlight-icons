...
#recolor the amule theme
cd $_basedir/App_themes/amule_dark
7z x amule_dark.zip -oamule_dark
rm -f amule_dark.zip
for _png3 in $(find "$_basedir/App_themes/amule_dark/amule_dark/" -type f -name "*.png"); do
	convert $_png3 -fill ''$newcolor'' -colorize 100% -alpha on $_png3
	optipng -nb -strip all $_png3
done
7z a amule_dark.zip amule_dark/*
rm -rf amule_dark
cd $_basedir
case $_dialog in
	yad)
